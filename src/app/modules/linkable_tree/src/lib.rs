...
// TODO investigate how storage works in substrate in detail

// TODO decide what code chunks should be reimplemented what to remain the same
type Pallet<T, I> = anchor::Pallet;
...
// TODO reimplement it for cosmos
...
// TODO replace wwith cosmos event
#[pallet::event]
...
// TODO keep this Error enum or use thiserror library
#[pallet::error]
pub enum Error<T, I = ()> {
...
// TODO figure out why this hooks are needed
#[pallet::hooks]
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
    ...
    pub fn create(origin: OriginFor<T>, max_edges: u32, depth: u8) -> DispatchResultWithPostInfo {
        ensure_root(origin)?;
        let tree_id =
            <Self as LinkableTreeInterface<_>>::create(T::AccountId::default(), max_edges, depth)?;
...
impl<T: Config<I>, I: 'static> LinkableTreeInterface<LinkableTreeConfigration<T, I>>
    for Pallet<T, I>
{
    fn create(creator: T::AccountId, max_edges: u32, depth: u8) -> Result<T::TreeId, Error> {
        let id = T::Tree::create(creator, depth)?;
        MaxEdges::<T, I>::insert(id, max_edges);
        Ok(id)
    }

    fn insert_in_order(id: T::TreeId, leaf: T::Element) -> Result<T::Element, Error> {
...
// TODO implement tests for linkable tree when all above TODOs are addressed
